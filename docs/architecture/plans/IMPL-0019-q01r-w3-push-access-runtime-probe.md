...
**Simplest correct fix: a runtime probe.** Before returning `can_push=True`, attempt `git ls-remote --push <https-with-token-url>` against the configured repo. If the wire-level handshake returns the repo's refs, the token can push. If it returns 403/permission-denied, return `can_push=False` with a precise reason. ~30 lines + 3 tests.
...
We pick the probe. `--push --dry-run` is even safer than a scratch-ref push: it never writes to the remote.
...
Backend (V2):
- `backend/cliff/integrations/github_app/client.py` — add a `_probe_git_push(token, repo_url) -> bool` helper that runs `git ls-remote --push https://x-access-token:{token}@github.com/{owner}/{repo}` via `asyncio.create_subprocess_exec` with a 5s timeout. Returns True on exit-code 0, False on anything else. Wrap `check_repo_push_access`'s currently-permissive return paths (those that say `can_push=True, reason=""`) so they invoke the probe and downgrade to `can_push=False, reason="<specific>"` if it fails.
...
async def test_check_runs_probe_when_user_perms_say_push_true(monkeypatch):
    ...
    # mock subprocess: git ls-remote --push returns exit 0
...
async def test_probe_timeout_returns_can_push_false_with_specific_reason(monkeypatch):
    # mock subprocess: hangs past 5s timeout
    result = await check_repo_push_access(...)
    assert result.can_push is False
...
- Drive a Critical through Approve & generate fix: if probe fails, preflight returns 412 within ~5s with "verified push access failed: <reason>" message in `error_details`. UI surfaces `executor_failed` (B35a/B35b path).
...
- **5s timeout might be too short for slow corporate networks.** Make it configurable via `CLIFF_PUSH_PROBE_TIMEOUT_SECONDS`, default 5.
- **The probe leaks the token to the local git process.** Already happens — `git clone` and `git push` in the executor use the same pattern. No new exposure.
...
Single PR, 2 commits:
1. `feat(q01r-w3-probe): runtime git-ls-remote probe for push access verification (B37)`
