...
Cliff ships as a single, signed Docker image published to the GitHub
Container Registry. The image runs as the non-root user `cliff`
...
2. Creates `~/cliff/` (override with `CLIFF_HOME=...`).
3. Downloads the release `docker-compose.yml`.
4. Writes a `.env` with a freshly generated `CLIFF_CREDENTIAL_KEY`.
...
```bash
curl -fsSL https://github.com/galanko/Cliff/releases/latest/download/install.sh \
  | CLIFF_NON_INTERACTIVE=1 ANTHROPIC_API_KEY=sk-ant-... sh
```
...
```bash
mkdir -p ~/cliff && cd ~/cliff

# Pin to a specific release; check the latest tag at
# https://github.com/galanko/Cliff/releases
VERSION=0.1.0-alpha
curl -fsSL "https://github.com/galanko/Cliff/releases/download/v${VERSION}/docker-compose.yml" \
  -o docker-compose.yml

# Generate a credential vault key (used for storing integration secrets)
echo "CLIFF_CREDENTIAL_KEY=$(openssl rand -base64 32)" > .env
echo "ANTHROPIC_API_KEY=sk-ant-..."                       >> .env
echo "CLIFF_VERSION=${VERSION}"                         >> .env
...
docker run --rm -p 8000:8000 \
  ...
  -e CLIFF_CREDENTIAL_KEY="$(openssl rand -base64 32)" \
  -v cliff-data:/data \
  ghcr.io/galanko/cliff:latest
```
...
> [release page](https://github.com/galanko/Cliff/releases), or run:
>
> ```bash
> docker buildx imagetools inspect ghcr.io/galanko/cliff:0.1.0-alpha
> ```
>
> then use:
>
> ```bash
> docker run ... ghcr.io/galanko/cliff@sha256:<digest>
> ```
...
| `CLIFF_VERSION`           | no       | `latest`| Image tag the compose file pulls (e.g. `0.1.0-alpha`).         |
| `CLIFF_APP_PORT`          | no       | `8000`  | Host port mapped to the container's `8000`.                    |
| `CLIFF_DATA_DIR`          | no       | `/data` | Where SQLite + workspace state live (inside the container).   |
| `CLIFF_CREDENTIAL_KEY`    | no       | —       | Base64-encoded 32-byte AES key for the integration vault.      |
| `CLIFF_DEMO`              | no       | `false` | Auto-seeds demo findings on first run.                         |

### Generating `CLIFF_CREDENTIAL_KEY`
...
Cliff, but you can't store integration credentials.

## Persisting data

The compose file uses a named Docker volume (`cliff-data`) by default.
...
```bash
mkdir -p /srv/cliff-data
sudo chown -R 10001:10001 /srv/cliff-data
docker run ... -v /srv/cliff-data:/data ghcr.io/galanko/cliff:latest
```
...
- **SELinux (Fedora, RHEL, CentOS Stream):** add `:Z` to your bind-mount
  flag so the container can read/write — `-v /srv/cliff-data:/data:Z`.
...
- File paths: keep `CLIFF_HOME` inside WSL (`~/cliff`) rather than
...
```bash
docker run --rm --user 0 -v cliff-data:/data alpine \
...
- **Port 8000 already in use** — set `CLIFF_APP_PORT=8001` in
  `.env`, then `docker compose up -d` again.
- **SBOM scanner says the image references unknown components** — the
  CycloneDX SBOM lives at `ghcr.io/galanko/cliff:sha256-<digest>.sbom`
