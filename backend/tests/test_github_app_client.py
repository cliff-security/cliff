...
# ---------------------------------------------------------------------------
# list_installations (ADR-0048)
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_list_installations_returns_parsed_entries():
    captured: dict = {}
    ...
    client = _client_with_handler(handler)
    result = await client.list_installations(access_token="ghu_test")

    assert all(isinstance(i, InstallationInfo) for i in result)
    assert [i.installation_id for i in result] == [100, 200]
    assert result[0].app_slug == "cliff-security"
    assert result[0].account_login == "octocat"
    assert result[0].account_type == "User"
    assert result[1].account_type == "Organization"
    # Paged at 100, authenticated with the user access token.
    assert "per_page=100" in captured["url"]
    assert captured["url"].endswith("/user/installations?per_page=100")
    assert captured["auth"] == "Bearer ghu_test"


@pytest.mark.asyncio
async def test_list_installations_empty_when_user_has_none():
    def handler(_request: httpx.Request) -> httpx.Response:
        return httpx.Response(200, json={"total_count": 0, "installations": []})

    client = _client_with_handler(handler)
    assert await client.list_installations(access_token="ghu_test") == []


@pytest.mark.asyncio
async def test_list_installations_raises_transient_on_5xx():
    """A 5xx must raise the transient subclass so the orchestrator retries
    rather than marking the flow terminal."""
...
async def test_list_installations_raises_on_401():
    ...
    client = _client_with_handler(handler)
    with pytest.raises(GitHubDeviceFlowError):
        await client.list_installations(access_token="bad")


@pytest.mark.asyncio
async def test_list_installations_raises_when_installations_key_missing():
    def handler(_request: httpx.Request) -> httpx.Response:
...
@pytest.mark.asyncio
async def test_list_installations_skips_malformed_entries():
    """One bad entry shouldn't sink the whole list — parse what we can."""
                ...
                "installations": [
                    {
                        "id": 1,
                        "app_slug": "cliff-security",
                        "account": {"login": "ok", "type": "User"},
                    },
                    {"app_slug": "no-id-here"},  # missing id → skipped
    ...
    result = await client.list_installations(access_token="ghu_test")
    assert len(result) == 1
    assert result[0].installation_id == 1
