...
Mirrors the conventions used by the other repos in ``cliff.db`` — async
aiosqlite, ISO 8601 timestamps as TEXT, async commits per write. Lives
under ``cliff.integrations.github_app`` rather than ``cliff.db``
...
if TYPE_CHECKING:
    ...
    from cliff.models import IntegrationConfig
...
async def list_other_enabled_github_integrations(
    ...
    from cliff.db.repo_integration import _row_to_integration
