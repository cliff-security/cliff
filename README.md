...
Built on [OpenCode](https://github.com/anomalyco/opencode). Runs natively on macOS and Linux, or in a single Docker container. AGPL-3.0.

> Alpha. Single-user. Expect rough edges — see [ROADMAP.md](ROADMAP.md).

## Quick start

**macOS / Linux (recommended)** — no Docker required, ~2 minutes:
...
Then open [http://127.0.0.1:8000](http://127.0.0.1:8000) and paste your Anthropic or OpenAI key in Settings.

Prereqs: `git`, `curl`, `gh` ([install gh](https://github.com/cli/cli#installation)). The installer fetches `uv`, a managed Python 3.11, the OpenCode binary, and the Trivy + Semgrep scanners. Run `opensec doctor` any time to diagnose.
...
Manual install, image verification, and platform notes: [docs/install.md](docs/install.md).
