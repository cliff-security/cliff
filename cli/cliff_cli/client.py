"""HTTP client for the Cliff daemon.

Wraps httpx with the conventions every CLI command needs:

  * base URL from ``CLIFF_URL`` (default ``http://127.0.0.1:8000``);
...
def _base_url() -> str:
    return os.environ.get("CLIFF_URL", "http://127.0.0.1:8000").rstrip("/")
        ...
        try:
            ...
            if exc.status == 404:
                raise VersionMismatchError(
                    "Cliff server is missing /api/version — older than this CLI supports.",
                    min_cli="unknown",
        ...
        if _parse_version(__version__) < _parse_version(min_cli):
            raise VersionMismatchError(
                (
                    ...
                    f"(min {min_cli}). Re-run the Cliff installer to upgrade."
                ),
                min_cli=min_cli,
                our_version=__version__,
            )
