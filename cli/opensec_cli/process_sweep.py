"""Owner-safe process discovery and cleanup for the OpenSec CLI.

Used by ``opensec stop`` / ``restart`` / ``uninstall`` to find leaked OpenSec
processes (parent uvicorn + child OpenCode binaries) when the parent died
abruptly and left orphans holding ports.

Hard ownership rule: a process is OpenSec-owned **iff** its cmdline matches
one of two patterns:

1. The parent uvicorn — argv contains the literal token ``uvicorn`` and the
   target ``opensec.main:app``.
2. Our installed opencode binary — ``argv[0]`` (or the executable path) is
   exactly ``$OPENSEC_HOME/bin/opencode``.
...
class FoundProcess:
    """An OpenSec process the user is allowed to signal."""
...
@dataclass(frozen=True)
class PortSquatter:
    """A non-OpenSec process bound to one of our ports. Report only — never signal."""
...
def _classify(
    ...
    has_app = any("opensec.main:app" in arg for arg in cmdline)
    if has_uvicorn and has_app:
        return "uvicorn"

    # Rule 2: our opencode binary at the exact installed path.
    target = str(opensec_opencode_bin)
    if cmdline[0] == target:
        return "opencode"
    try:
        exe = proc.exe()
...
def find_opensec_processes(opensec_home: Path) -> list[FoundProcess]:
    """Find every OpenSec-owned process running as the current user.
    ...
    """
    opencode_bin = opensec_home / "bin" / "opencode"
